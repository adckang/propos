...
function loadModule() {
  delete require.cache[require.resolve('../../src/utils/settlementSchedule.js')];
  return require('../../src/utils/settlementSchedule.js');
}
...
describe('settlementSchedule — TC-MS-001 ~ TC-MS-007', () => {
  ...
  test('TC-MS-001: 1일이면 isSettlementDay() === true', () => {
    const { isSettlementDay } = loadModule();
  ...
  test('TC-MS-003: getSettlementPeriod() — 1월이면 전년도 12월 반환', () => {
    const { getSettlementPeriod } = loadModule();
