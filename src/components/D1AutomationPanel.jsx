  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%',
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)',
        padding:'14px 20px', flexShrink:0, boxShadow:'var(--shadow)' }}>

        <div style={{ display:'flex', alignItems:'center', gap:14,
          marginBottom: results.length > 0 ? 10 : 0 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:'var(--text3)', marginTop:2,
              ...
              <span style={{ color:countdown.secs<300?'#dc2626':countdown.secs<3600?'#d97706':'#059669', fontWeight:700 }}>
                ⏱ {countdown.label}
              </span>
              {/* 데이터 소스 뱃지 */}
              <span style={{ fontSize:9, fontWeight:700, padding:'1px 6px', borderRadius:4,
                ...
                border: `1px solid ${isRealData ? '#a7f3d0' : '#e2e8f0'}` }}>
          ...
          <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:8 }}>
            ...
            <button onClick={handleSync} disabled={syncing || !propConfigs.some(c=>c.icalUrl)}
              style={{ padding:'6px 12px', borderRadius:6,
                background: (syncing||!propConfigs.some(c=>c.icalUrl)) ? 'var(--border)' : '#ecfdf5',
                border:`1.5px solid ${(syncing||!propConfigs.some(c=>c.icalUrl)) ? 'var(--border2)' : '#a7f3d0'}`,
                color: (syncing||!propConfigs.some(c=>c.icalUrl)) ? 'var(--text3)' : '#059669',
            ...
            <button onClick={() => setView(v => v==='settings'?'auto':'settings')}
              style={{ padding:'6px 12px', borderRadius:6,
                ...
                color: view==='settings' ? '#fff' : 'var(--text3)',
            ...
            {view === 'auto' && !haMode && (
              <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:11,
                color:'var(--text3)', cursor:'pointer', userSelect:'none' }}>
            ...
            {view === 'auto' && (
              <button onClick={handleRun} disabled={running}
                style={{ padding:'8px 18px', borderRadius:7,
                  background:running?'var(--border)':'var(--blue)',
                  border:`1.5px solid ${running?'var(--border2)':'#1d4ed8'}`,
                  color:running?'var(--text3)':'#fff', fontSize:13, fontWeight:700,
                  cursor:running?'not-allowed':'pointer',
                  fontFamily:"'DM Sans',sans-serif", transition:'all 0.15s',
                  display:'flex', alignItems:'center', gap:6 }}>
                {running
                  ? <><span style={{ display:'inline-block', animation:'d1spin 1s linear infinite' }}>↻</span> 처리중...</>
                  : '▶ 지금 실행'}
              </button>
            )}
          </div>
        ...
        {view === 'auto' && results.length > 0 && (
          <div style={{ display:'flex', gap:16, paddingTop:10,
            ...
            {[
              { label:'대상',     val:targets.length,   color:'var(--text3)' },
              { label:'완료',     val:summary.success,  color:'var(--green)' },
              { label:'부분완료', val:summary.partial,  color:'var(--yellow)' },
              { label:'실패',     val:summary.failed,   color:'var(--red)'   },
            ].map(({ label, val, color }) => (
              <div key={label} style={{ display:'flex', alignItems:'baseline', gap:4 }}>
                <span style={{ fontFamily:"'DM Mono',monospace", fontSize:18, fontWeight:700, color }}>{val}</span>
                <span style={{ fontSize:11, color:'var(--text3)' }}>{label}</span>
              </div>
            ))}
            {lastRun && (
              <div style={{ marginLeft:'auto', fontSize:11, color:'var(--text4)', fontFamily:"'DM Mono',monospace" }}>
                마지막 실행 {lastRun}
              </div>
            )}
          </div>
