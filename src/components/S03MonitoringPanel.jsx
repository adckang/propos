...
// ── Mock Infrastructure ──────────────────────────────────────
function makeMockSensorInfra(haMode = false) {
  // Mock: 30초마다 약간씩 변동하는 센서값 반환
  ...
  return { getSensorStates, getCallCount: () => callCount };
}
...
function PropMonitorBadge({ prop, readings, onSelect, selected }) {
  ...
  const T = _s03.getDefaultThresholds();
  let anomaly = null;
  if (r) {
    const a = _s03.isAnomaly(r, T);
...
function S03MonitoringPanel({ onBack }) {
  ...
  // ── 인프라 선택 ──
  function getInfra() {
    return haMode ? _haS03 : makeMockSensorInfra(false);
  }
  ...
  async function pollOneProp(prop, infra) {
    try {
      ...
      const anomaly = _s03.isAnomaly(reading, T);
